...
class DwarfBridge:
    ...
    def _parse_struct(self, DIE, file_index: dict[int, str], dwarf, CU) -> StructLayout | None:
        ...
        for child in DIE.iter_children():
            ...
            if not member_name_attr or not offset_attr:
                continue
            ...
            # DW_AT_data_member_location can be a constant (offset in bytes)
            # or a location expression — for simple structs it's always a constant
            try:
    ...
    def _cache_path(self) -> Path:
        vmlinux_mtime = int(self.vmlinux_path.stat().st_mtime) if self.vmlinux_path.exists() else 0
        return self.cache_dir / f"dwarf_{vmlinux_mtime}.pkl"
