    ...
    def vector_search(
        ...
        for i, node_id in enumerate(results["ids"][0]):
            ...
            node = CodeNode(
                ...
                code=results["documents"][0][i],  # stored as document text
                calls=meta.get("calls", "").split(",") if meta.get("calls") else [],
                uses_structs=meta.get("uses_structs", "").split(",") if meta.get("uses_structs") else [],
                includes=meta.get("includes", "").split(",") if meta.get("includes") else [],
            )
